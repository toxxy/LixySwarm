...
| Distributed training contribution | Partial | A worker fetches a safe NumPy token artifact, requires the exact locally loaded checkpoint hash, computes a bounded real gradient, and returns a verified NPZ artifact without applying it. Three-to-31-peer quorum mode validates candidates, verifies portable worker receipts, and creates a streaming coordinate median. Sybil-independent membership, poisoning/privacy defenses, and collective promotion governance are missing. |
