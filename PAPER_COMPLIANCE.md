...
| Distributed training contribution | Partial | A worker fetches a safe NumPy token artifact, requires the exact locally loaded checkpoint hash, computes a bounded real gradient, and returns a verified NPZ artifact without applying it. Three-to-31-peer quorum mode validates candidates, creates a streaming coordinate median, and issues each included worker a duplicate-resistant credit containing both the worker receipt and requester attestation. Connected peers present bounded evidence; scheduling prefers firsthand credits and issuer diversity while periodic aged-identity exploration gives newcomers a path to their first credit. Sybil-independent issuer trust, poisoning/privacy defenses, and collective promotion governance are missing. |
