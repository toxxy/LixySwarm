...
| Internet bootstrap and seed independence | Implemented in v3 code; initial public operation | A fresh client includes the first operated public seed. Saved peers plus seed endpoints feed `PeerManager`; both in-process and separate-interpreter three-node tests prove encrypted direct communication continues after graceful or abrupt seed shutdown. Redundant independent seeds, official DNS seed infrastructure, and public multi-region endurance evidence are not configured yet. |
| VPS seed path | Implemented and initially operated | `node_daemon.py` runs as a hardened non-root service on the first public VPS seed. External TCP reachability, encrypted LSP v3 handshake, persistent identity, and restart behavior were validated. It introduces peers and is not a required relay. Multi-region endurance and a second independent operator remain. |
...
| Distributed inference contribution | Partial | Signed-origin typed work selects consenting peers and executes an allowlisted full-request inference handler. Remote prompts cannot access/write personal Matriarca, history, or Dolphin state. Verified mode runs deterministically on three-to-nine exact-model/coarse-group-diverse peers and requires an exact majority with signed receipts. Persistent newcomer exploration prevents strict reputation starvation. Inbound work/identity rates are bounded; authenticated cancellation is polled per token; automatic single-peer jobs have same-ID fallback across distinct eligible peers. Verified quorums replace bounded failed members under one deadline without reducing accepted cardinality and expose attempt counts. Sybil independence, cross-hardware determinism evidence, durable fair-share, and killable process sandboxing remain missing. |
| Compute identity cost | Partial | Requesters can optionally require a persisted SHA-256 work stamp bound to the worker's Ed25519 key. The default is zero because expendable hashing is an admission defense, not the intended contribution reward; it does not prove operator independence. |
...
| Distributed training contribution | Partial | A worker fetches a safe NumPy token artifact, requires the exact locally loaded checkpoint hash, computes a bounded real gradient, and returns a verified NPZ artifact without applying it. Three-to-31-peer quorum mode validates candidates, replaces bounded failed workers while retaining exact cardinality, creates a streaming coordinate median, and issues each included worker a duplicate-resistant credit containing both the worker receipt and requester attestation. Compute and artifact transfer share one deadline/cancellation signal. Connected peers present bounded evidence; scheduling prefers firsthand credits and issuer diversity while periodic aged-identity exploration gives newcomers a path to their first credit. Sybil-independent issuer trust, poisoning/privacy defenses, and collective promotion governance are missing. |
| Signed model releases and rollback | Partial | Threshold Ed25519 manifests, local trust policies, optional pinned genesis, revocation, monotonic activation, weights-only runtime loading, and explicit rollback exist. Official keys/genesis, network discovery, key recovery/rotation, and decentralized promotion policy do not. |
...
The repository supports the paper's central prototype thesis and now includes a post-paper encrypted LSP v3 topology plus replicated inference, artifacts, gradient quorum/median, dual-signed useful-work evidence and firsthand scheduling priority, diversity, and local-ban controls. It does **not** yet satisfy the paper's target Internet-scale compute ecosystem because pseudonymous third-party issuers still undermine quorum independence, work lacks process isolation/attestation, and governance, key rotation/review, and redundant independent seed operations remain. The correct current stage remains **Infant / research prototype**.
